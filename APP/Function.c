...
void HEX2ASCII(uint8_t *mdata,const uint8_t *sdata,uint16_t len)
{
  ...
  for(i=0;i<len;i++)
	{
   mdata[i*2]=HEX_Table[(sdata[i]>>4)&0x0f];
   mdata[i*2+1]=HEX_Table[sdata[i]&0x0f];	
  }
	return;
}
...
/********************************************
*函数功能：ASCII码转HEX码
*入口参数：mdata--目标数据，转换后的HEX
          sdata--源数据，转换前的ASCII
          len  --待转换数据的长度
*出口参数：none
*函数说明：2个ASCII产生1个HEX
*********************************************/
void ASCII2HEX(uint8_t *mdata, const uint8_t *sdata,uint16_t len)
{
  uint16_t i;
  for(i=0;i<len;i++)
	{
   *(mdata+i)=(ASCII_Table[sdata[i*2]]<<4)&0xf0;
   *(mdata+i)+=ASCII_Table[sdata[i*2+1]];	
  }
	return;
}
...
/********************************************
*函数功能：16位CRC校验
*入口参数：puchMsg--待CRC计算数据
          usDataLen--待校验数据长度
*出口参数：2byteCRC校验值
*函数说明：用于串口4通信数据的CRC校验
*********************************************/
...
*入口参数：data--待计算数据
*出口参数：和校验值
*函数说明：用于串口1通信数据的lenth数据和校验
*********************************************/
uint16_t LCHKsum(uint16_t data) 
{
	uint16_t sum;
	sum=(data&0x000F)+((data>>4)&0x000F)+((data>>8)&0x000F);
	sum=sum%16;
	sum=(~sum+1)&0x000F;
	sum=(sum<<12)|data;
	return sum;
